...
namespace Uni.Scan.Client.Pages.Printing
{
    public partial class Labels
    {
        ...
        private async Task PrintSingleLabel(int id)
        {
            var parameters = new DialogParameters();
            ModalType = "SingleLabel";
            parameters.Add(nameof(PrintModal.ModalType), ModalType);
            parameters.Add(nameof(PrintModal.intID), id);
        ...
        private async Task InvokeAddLabelModal(int id = 0)
        {
            ...
            {
                var options = new DialogOptions { NoHeader = true, FullScreen = true, FullWidth = true, DisableBackdropClick = true };
        ...
        private async Task Filter()
        {
            ...
            if (LabelsType == 2)
            {
                Elements = FilterElements.Where(u => u.Type.Equals(LabelType.Free)).Select(u => u).ToList();
            }

            if (LabelsType == 3)
            {
                Elements = FilterElements.Where(u => u.Type.Equals(LabelType.Linked)).Select(u => u).ToList();
            }
        }

        private bool Search(LogisticTaskLabelDTO lbl)
        {
            if (string.IsNullOrWhiteSpace(searchString)) return true;
            if (lbl.ProductId?.Contains(searchString, StringComparison.OrdinalIgnoreCase) == true)
            {
                return true;
            }

            if (lbl.ProductName?.Contains(searchString, StringComparison.OrdinalIgnoreCase) == true)
            {
                return true;
            }

            if (lbl.IdentifiedStock?.Contains(searchString, StringComparison.OrdinalIgnoreCase) == true)
            {
                return true;
            }

            return lbl.ProductId?.Contains(searchString, StringComparison.OrdinalIgnoreCase) == true;
        }
        ...
        {
            await LogisticLabelDialog.ShowDialogAsync(_dialogService);
        }
    }
}