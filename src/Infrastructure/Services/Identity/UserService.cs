	...
	public class UserService : IUserService
	{
		...
		public async Task<IResult> RegisterAsync(RegisterRequest request, string origin)
		{
			...
			if (userWithSameEmail == null)
			{
				...
				{
					return await Result.FailAsync(result.Errors.Select(a => _localizer[a.Description].ToString())
						.ToList());
				}
			}
			else
			{
				return await Result.FailAsync(string.Format(_localizer["Email {0} is already registered."],
					request.Email));
			}
		}

		private async Task<string> SendVerificationEmail(UniUser user, string origin)
		{
		...
		public async Task<IResult> ToggleUserStatusAsync(ToggleUserStatusRequest request)
		{
			...
			if (isAdmin)
			{
				return await Result.FailAsync(
					_localizer["Administrators Profile's ProcessingStatusCode cannot be toggled"]);
			}
		...
		public async Task<string> ExportToExcelAsync(string searchString = "")
		{
			...
			var result = await _excelService.ExportAsync(users, sheetName: _localizer["Users"],
				mappers: new Dictionary<string, Func<UniUser, object>>
				{
