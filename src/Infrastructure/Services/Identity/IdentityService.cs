...
namespace Uni.Scan.Infrastructure.Services.Identity
{
    public class IdentityService : ITokenService
    {
        ...
        private readonly SignInManager<UniUser> _signInManager;
        private readonly IStringLocalizer<IdentityService> _localizer;

        public IdentityService(
            UserManager<UniUser> userManager, RoleManager<UniRole> roleManager,
            IOptions<AppConfiguration> appConfig, SignInManager<UniUser> signInManager,
        ...
        private async Task<string> GenerateJwtAsync(UniUser user)
        {
            var token = GenerateEncryptedToken(GetSigningCredentials(), await GetClaimsAsync(user));
            return token;
        }

        private async Task<IEnumerable<Claim>> GetClaimsAsync(UniUser user)
        {
            ...
            var claims = new List<Claim>
                {
                    ...
                    new(nameof(UniUser.SiteID), user.SiteID),
                    new(nameof(UniUser.EmployeeID), user.EmployeeID)
                }
                .Union(userClaims)
                .Union(roleClaims)
